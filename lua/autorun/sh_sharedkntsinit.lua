...
if SERVER then
    if not ConVarExists("sv_kaito_free_nvg") then
        CreateConVar("sv_kaito_free_nvg", 0, FCVAR_ARCHIVE, "Player's do not need to use entity")
    end
    if not ConVarExists("sv_kaito_free_thermal_only") then
        CreateConVar("sv_kaito_free_thermal_only", 0, FCVAR_ARCHIVE, "Thermals will not need entity only")
    end

    if not ConVarExists("sv_kaito_free_nightvision_only") then
        CreateConVar("sv_kaito_free_nightvision_only", 0, FCVAR_ARCHIVE, "Nightvision will not need entity only")
    end    
end
...
net.Receive("kaito_net_nightvision_varbridge", function()

    if SERVER then return end
    local isNVGEquipped = LocalPlayer():GetNWInt("nvg_equipped",0)
    local isNVGFree = net.ReadInt(8)
    local isFree = net.ReadInt(8)
...
if CLIENT then
    concommand.Add("enable_night_vision", function(ply,cmd,args,argStr)
        if timeout < CurTime() then

            if not visionNocturneActive then
            net.Start("kaito_net_start_nvg", false)
                net.WriteEntity(ply)
            net.SendToServer()
                timeout = CurTime()+2
            else
...
concommand.Add("enable_thermal_vision", function(ply,cmd,args,argStr)
    if timeout < CurTime() then
